...
@interface NotifyViewController () <UIWebViewDelegate, MxAlertViewDelegate, LoginViewDelegate>
...
- (void)replytBtnClicked:(id)sender {
    ...
    if (!user.isLogin) {
        MxAlertView *alert = [[MxAlertView alloc]initWithTitle:nil message:@"用户未登录，是否登录" delegate:nil cancelButtonTitle:@"登录" otherButtonTitles:@"取消"];
        alert.tag = 0x500;
        alert.delegate = self;
        [alert show:self];
        return;
    }
    NSDictionary *params = @{@"role":@"user", @"user_id":user.user_id, @"kind":self.msg.kind, @"msg_id":self.msg.msgid, @"content":textView.text, @"seller_id":self.msg.seller_id};
    
    __block NotifyViewController *blockself = self;
    [self.activity startAnimating];
    [RCar POST:rcar_api_user_msg modelClass:nil config:nil params:params success:^(NSDictionary *model) {
...
- (void) loadMessageDetail {
    ...
    __block NotifyViewController *blockself = self;
    [self.activity startAnimating];
    [RCar GET:rcar_api_user_msg modelClass:@"MessageDetailModel" config:config params:params success:^(MessageDetailModel *model) {
        [blockself.activity stopAnimating];
...
- (void)alertView:(nullable MxAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView.tag == 0x500 && buttonIndex == 0) {
