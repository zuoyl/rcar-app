...
@interface MyOrderDetailViewController () <MxAlertViewDelegate>
@end
...
- (void)loadOrderDetail {
    ...
    DCArrayMapping *mapper = [DCArrayMapping mapperForClassElements:[SellerServiceModel class] forAttribute:@"service_list" onClass:[MyOrderDetailModel class]];
    DCParserConfiguration *config = [DCParserConfiguration configuration];
    [config addArrayMapper:mapper];
    
    
    NSDictionary *params = @{@"role":@"seller", @"seller_id":seller.seller_id, @"order_id":self.order_id, @"scope":@"seller"};
    __block MyOrderDetailViewController *blockself = self;
    
    [RCar GET:rcar_api_seller_order modelClass:@"MyOrderDetailModel" config:config params:params success:^(MyOrderDetailModel *dataModel) {
...
- (void)setOrderStatus:(NSString *)status info:(NSDictionary *)info {
    ...
    if (self.user_id != nil && ![self.user_id isEqualToString:@""])
