...
- (UIView*)bannerView {
    ...
    if (!_bannerView) {
        ...
        UILabel *noConnectionLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.origin.x, 0, self.view.frame.size.width, MM_DefaultBannerHeight)];
        ...
        [noConnectionLabel setText:@"无网络连接，请检查网络"];
...
- (void)setMode:(ReachabilityMode)mode {
...
- (void)showBanner {
    ...
    switch (self.mode) {
        ...
        case ReachabilityModeOverlay: {
            ...
            if (view.frame.origin.y == -view.frame.size.height) {
                ...
                viewFrame.origin.y += view.frame.size.height + 60;
    ...
    [UIView animateWithDuration:MM_AnimationDuration
                     ...
                     completion:^(BOOL finished) {
                         ...
                         if (self.mode == ReachabilityModeResize) {
...
- (void)cancel {
    ...
    [ReachabilityViewController cancelPreviousPerformRequestsWithTarget:self selector:@selector(showBanner) object:nil];
    [ReachabilityViewController cancelPreviousPerformRequestsWithTarget:self selector:@selector(hideBanner) object:nil];
}
