...
- (void)loadImages:(NSArray *)imageNameArray {
    if (!imageNameArray || imageNameArray.count == 0)
        return;
    
    for (int i = 0; i < imageNameArray.count && i < self.maxOfImage; i++) {
        // create photo and thumbnail
        NSString *urlstr = [[RCar imageServer] stringByAppendingString:imageNameArray[i]];
        urlstr = [urlstr stringByAppendingString:@"?target=seller"];
        
        MWPhoto *photo = [MWPhoto photoWithURL:[[NSURL alloc]initWithString:urlstr]];
