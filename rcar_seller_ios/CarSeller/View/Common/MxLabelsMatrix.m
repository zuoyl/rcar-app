...
- (void)tableTapClicked:(UITapGestureRecognizer *)tap {
    UITextField *textField = tap.view;
...
- (void)addRecord: (NSArray*)record {
    ...
    for(uint i = 0; i < record.count; i++){
        ...
        col1.textAlignment = NSTextAlignmentLeft;
        ...
        if ([record[i] isKindOfClass:[UIView class]]) {
            UIView *view = [record objectAtIndex:i];
            if (view.frame.size.width == 0 || view.frame.size.height == 0)
                [view setFrame:CGRectMake(colWidth - colWidth/2, 0, colWidth/2, rowHeight)];
            view.tag = self.numRows;
            view.exclusiveTouch = YES;
            [col1 addSubview:view];
            col1.userInteractionEnabled = YES;
            
            [col1 addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tableTapClicked:)]];
